# Mustache is a simple, fast way to get more Mustache into your CoffeeScript and
# Javascript.
#
...
Renderer = (template, delimiters = ['{{','}}'], section = null) ->
    ...
    while match = tagPattern.exec(template)
        ...
        switch type
                ...
                buildInterpolationTag = (name, is_unescaped) ->
                    return (context) ->
                        if (value = Find(name, context) ? '') instanceof Function
                            value = Expand(@, Renderer("#{value()}"), arguments...)
            ...
            when '>'
                buildPartialTag = (name, indentation) ->
                    return (context, partials) ->
                        ...
                        return Expand(@, Renderer(partial), arguments...)
            ...
            when '#', '^'
                ...
                [tmpl, pos] = Renderer(template, delimiters, sectionInfo)
                ...
                sectionInfo['^'] = buildInvertedSectionTag = (name, delims, raw) ->
                    return (context) ->
                        ...
                        value = if value.length is 0 then Renderer(raw, delims) else []
                        return Expand(@, value, arguments...)
...
@Mustache =
    ...
    escape: (value) ->
        return String(value).replace(/[&"<>]/g, (character) -> "&#{ ESCAPED_ENTITIES[character] };")
    ...
    render: (template, data, partials = null) ->
        ...
        return Expand(@, Renderer(template), context.concat([data]), partials)
